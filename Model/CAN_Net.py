...
class RFBlock(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(RFBlock, self).__init__(init_cfg=init_cfg)
...
class CAN_net(BaseBackbone):#feature extraction
    """ResNet backbone.
    ...
    def __init__(self,
        ...
        super(CAN_net, self).__init__(init_cfg)
        ...
        self.casc_att = CASC(64,64)

        self.Non_local1 = Non_local(128)
        self.Non_local2 = Non_local(256)
    ...
    def init_weights(self):
        super(CAN_net, self).init_weights()
        ...
        if self.zero_init_residual:
            for m in self.modules():
                ...
                elif isinstance(m, RFBlock):
                    constant_init(m.norm2, 0)

    def forward(self, x):
        if self.deep_stem:#layer(0)
            x = self.stem(x)
        else:
            x = self.conv1(x)
            x = self.norm1(x)
        ...
        x = self.maxpool(x)
        outs = []
        for i, layer_name in enumerate(self.res_layers):  # i: 0,1,2,3 -> layer 1,2,3,4
            RF_layer = getattr(self, layer_name)
            if i == 0:#layer(1)
                x = RF_layer(x)
                x = self.casc_att(x)#CASC_Att

            if i == 1:#layer(2)
                x = RF_layer(x)
                x = self.Non_local1(x)#NL

            if i == 2:#layer(3)
                x = RF_layer(x)
                x = self.Non_local2(x)#NL
